...
from app.users_bd import Users, Teams, TeamMembers
...
def add_team_member(teamname:str, username:str, db: Session=Depends(get_db), current_user: Users = Depends(get_current_user)):
    ...
    new_member = TeamMembers(teamname=teamname, username=user_to_add.username)
    db.add(new_member)
    db.commit()

    return {"message": f"Пользователь {username} успешно добавлен в команду"}